...
def path_to_seq(orient, path):
    ...
    global_instructions = encode_directions(path)
    car_instructions = []
...
def instructions_for_car_from_current_pos_to_dest(pos_car, dest_car, orientation, obst):
    """
    ...
    end = (int((100 - dest_car[1]) / 10), int((dest_car[0] + 100) / 10))

    grid = generateGrid(obst)
    path = shortest_path(start, end, grid)
    instr = path_to_seq(orientation, path)
...
# Test
num_obstacles = 500
obstacle_coor = np.random.randint(-100, 101, size=(num_obstacles, 2))
instructions_for_car_from_current_pos_to_dest((23, 45), (-50, -50,), 0,
                                              obstacle_coor)
